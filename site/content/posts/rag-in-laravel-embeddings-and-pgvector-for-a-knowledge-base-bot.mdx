...
In the [last post](/blog/streaming-ai-responses-in-laravel-with-server-sent-events) we streamed AI responses over SSE. Now let's fix the problem every chatbot hits in week one: the model doesn't know *your* data. Ask it about your refund policy or your API docs and it either hallucinates or shrugs.
...
Retrieval makes the model *knowledgeable*. The next step makes it *capable*: in part 4 we [build AI agents in PHP](/blog/building-ai-agents-in-php-tool-calling-with-laravel) - giving the model tools it can call (search orders, create tickets, query your database) and handling the tool-calling loop safely in Laravel. That's where chatbots turn into products.
